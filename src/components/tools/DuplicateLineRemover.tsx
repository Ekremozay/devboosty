    ...
    <div className="space-y-5">
      <div className="flex flex-wrap gap-4 items-center">
        {[['caseSensitive', 'Case-sensitive matching', caseSensitive, () => setCaseSensitive(v => !v)], ['trimLines', 'Trim whitespace', trimLines, () => setTrimLines(v => !v)]].map(([k, l, v, fn]) => (
          <label key={k as string} className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="space-y-2"><label className="text-sm font-semibold text-slate-700">Input ({input.split('\n').filter(Boolean).length} lines)</label>
        ...
        <div className="space-y-2"><div className="flex justify-between"><label className="text-sm font-semibold text-slate-700">Unique Lines ({output.split('\n').filter(Boolean).length})</label>
