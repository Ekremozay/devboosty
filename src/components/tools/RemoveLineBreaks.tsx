    ...
    <div className="space-y-5">
      <div className="flex gap-1 p-1 bg-slate-100 rounded-xl w-fit">
        {[['join', 'Join all into one line'], ['single', 'Remove extra blank lines'], ['double', 'Double-space paragraphs']].map(([k, l]) => (
          <button key={k} onClick={() => setMode(k as 'join'|'single'|'double')} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${mode === k ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{l}</button>
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="space-y-2"><label className="text-sm font-semibold text-slate-700">Input</label>
        ...
        <div className="space-y-2"><div className="flex justify-between"><label className="text-sm font-semibold text-slate-700">Output</label>
