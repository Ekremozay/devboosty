  ...
  return (
    <div className="space-y-5">
      <div className="border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer hover:border-brand-300 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all" onClick={() => inputRef.current?.click()}>
        ...
        <p className="font-semibold text-slate-700 dark:text-slate-300">Click to upload {isJpgToPng ? 'JPG/JPEG' : 'PNG'} image</p>
        <p className="text-sm text-slate-400 mt-1">All conversion happens locally in your browser</p>
      </div>
      {preview && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div className="space-y-2"><label className="text-sm font-semibold text-slate-600 dark:text-slate-400">Original ({file?.name})</label>
            <img src={preview} alt="Original" className="rounded-xl border border-slate-100 dark:border-slate-700 w-full object-contain max-h-64" /></div>
