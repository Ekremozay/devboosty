    ...
    <div className="space-y-5">
      <div className="flex gap-1 p-1 bg-slate-100 dark:bg-slate-700 rounded-xl w-fit">
        {(['encode', 'decode'] as Mode[]).map(m => (
          <button key={m} onClick={() => { setMode(m); process(input, m); }}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-all ${mode === m ? 'bg-white text-slate-800 shadow-sm dark:bg-slate-900 dark:text-white' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="space-y-2">
          <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">{mode === 'encode' ? 'Plain URL / Text' : 'Encoded URL'}</label>
        ...
        <div className="space-y-2">
          <div className="flex justify-between"><label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Output</label>
          ...
          <textarea className="tool-textarea bg-slate-50 dark:bg-slate-900/50" value={output} readOnly style={{ minHeight: 200 }} />
        </div>
      </div>
